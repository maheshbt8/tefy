...
<div class="">
<table class="table table-striped table-hover table-responsive " id="tableExport">
        <thead>
            <tr>
                <th>Seq.</th>
                <th>Name</th>
		...
		<div class="row">
			...
			<div class="col-lg-6 col-md-12">
				<div class="dashboard-list-box with-icons margin-top-20">
					<h4>Today Admissions</h4>
					<ul>
						<?php
						$ad_where=array('created_at >='=>date('Y-m-d').' 00:00:00','created_at <='=>date('Y-m-d').' 23:59:59','row_status'=>'1');
						$res=$this->common_model->select_results_info('admissions',$ad_where,'id DESC')->result_array();
					foreach ($res as $row) {
						$r_admission_status='';
                      if($row['admission_status']==1){
                        $r_admission_status='Application Submitted';
                      }elseif($row['admission_status']==2){
                        $r_admission_status='Application is being reviewed';
                      }elseif($row['admission_status']==3){
                        $r_admission_status='Admission Approved';
                      }elseif($row['admission_status']==4){
                        $r_admission_status='Admission Fee Payment';
                      }elseif($row['admission_status']==5){
                        $r_admission_status='Admission Confirmed';
                      }elseif($row['admission_status']==6){
                        $r_admission_status='Admission Canceled';
                      }
						?>
						<li>
							<i class="list-box-icon sl sl-icon-layers"></i> Application ID: <strong><a href="<?=base_url('admin/admission_form_view/').$row['id'];?>"><?=$row['application_id'];?></a></strong> <?=$r_admission_status;?>!
